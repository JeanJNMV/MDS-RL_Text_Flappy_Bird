...
\renewcommand\section{\@startsection{section}{1}{\z@}%
  {-10pt \@plus -3pt \@minus -3pt}% space BEFORE
  {5pt \@plus 5pt}% space AFTER
...
The objective of this project is to apply reinforcement learning (RL) techniques to a simplified, terminal-based version of Flappy Bird called Text Flappy Bird (TFB). In this game, an agent controls a character navigating through pipe obstacles to maximize survival time and score. 

We consider two variants of the environment, simple and screen, implemented using a custom OpenAI Gym framework, each with different levels of complexity and observation spaces. The problem is modeled as a Markov Decision Process and we explore two RL algorithms: Monte Carlo Control and Sarsa($\lambda$). This report evaluates and compares their performance and convergence across both environments, along with a sensitivity analysis of key hyperparameters and an examination of policy transferability.
...
\paragraph{Performance Comparison between Monte Carlo Control and SARSA.} Agents were trained for 25\,000 episodes and evaluated over 200 episodes (fewer for tile coding due to slower inference) on the setup (\texttt{height=15}, \texttt{width=20}, \texttt{pipe\_gap=4}), with a 50\,000-step cap per episode (max score: 4999). Figure~\ref{fig:train_scores} shows training curves and Table~\ref{tab:results} reports results. Due to a heavily skewed score distribution, the median better reflects typical performance than the mean, which is influenced by rare high scores and often too close to the standard deviation.
...
In the simple environment, both agents learned effective policies, achieving the maximum score of 4999 with zero variance. However, MCC showed to be more unstable, as some runs failed to converge during the experiments, while Sarsa consistently reached its optimal performance across all runs. This could be attributed to Sarsa's use of eligibility traces, which can help propagate rewards more efficiently and stabilize learning, while MCC relies on complete episode returns that can introduce higher variance in updates. In the screen environment, tile coding proved to be essential. Both MCC and Sarsa with tile coding achieved maximal evaluation performance, whereas hash-based agents failed to generalize across the large state space, producing much lower scores. 
...
This study shows that both Monte Carlo Control and Sarsa can learn effective policies in TextFlappyBird. Feature representation plays a critical role, as tile coding enables strong generalization while hash-based methods fail in high-dimensional settings. Finally, limited transferability across environment variations highlights the need for more robust function approximation methods to handle increased complexity.
